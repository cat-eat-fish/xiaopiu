<template>
	<view>
		<view class="tabr" :style="{top:headerTop}">
			<view :class="{on:typeClass === item.py}" @tap="switchType(item.py)" v-for="(item,index) in tabr" :key="index">{{item.text}}</view>
		</view>
		<view class="tabr" :style="{top:headerTop}">
			<view :class="{on:typeClass=='goods'}" @tap="switchType('goods')">商品</view>
			<view :class="{on:typeClass=='shop'}"  @tap="switchType('shop')">店铺</view>
		</view>
...
<script>
	export default {
		data() {
			return {
				tabr:[
					{id:0,text:"商品",py:"goods"},
					{id:1,text:"店铺",py:"shop"},
				],
				goodsList:[
