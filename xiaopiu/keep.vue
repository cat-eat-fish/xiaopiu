...
<template>
	<view class="content">
		<cat-tab @change="tagger" :initIndex="initIndex" :hasan="true" :navList="navList" bg="#333">
			<view class="showCon" v-if="initIndex == 0">
					...
					<view class="row" v-for="(row,index) in goodsList" :key="index">
						...
						<view class="carrier" :class="[typeClass=='goods'?theIndex==index?'open':oldIndex==index?'close':'':'']"
						 @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
							<view class="goods-info" @tap="toGoods(row)">
								<view class="img">
									<image src="https://www.jb51.net/images/logo.gif" mode="heightFix"></image>
								</view>
								<view class="info">
									<view class="title">{{row.name}}</view>
									<view class="price-number">
			...
			<view class="showCon" v-if="initIndex == 1">
				<view class="sub-list goods" >
					<view class="tis" v-if="goodsList.length==0">没有数据~</view>
					<view class="row" v-for="(row,index) in goodsList" :key="index">
						<!-- 删除按钮 -->
