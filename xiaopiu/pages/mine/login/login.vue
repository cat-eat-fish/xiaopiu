<template>
	<view>
		...
		<view class="form">
			<view class="username">
				<input placeholder="请输入手机号" v-model="phone" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="password">
			...
			<view class="res">
				<view @tap="toRegister">用户注册</view>
				<view @tap="toresetPwd">忘记密码</view>
			</view>
		</view>
		<!-- 第三方登录 -->
	...
	export default {
		...
		methods: {
			...
			doLogin(){
				...
				setTimeout(()=>{
					let md5PW = md5(this.password)
					uni.getStorage({
						key: 'UserList',
						success: (res)=>{
							for(let i in res.data){
								let row = res.data[i];
								if(row.username==this.phone){
									uni.hideLoading()
									//比对密码
									var data = {nickname: '测试',portrait:"",userName:this.phone,password:md5PW}
